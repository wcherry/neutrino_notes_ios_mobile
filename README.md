...
| Epic 22 | Sharing & Permissions | COMPLETE |
...
The app is structured as a five-tab SwiftUI shell with tabs for Notes, Recents, Favorites, Offline, and Settings. Each tab keeps its own `NavigationStack` and navigation state; the Notes tab owns an explicit `NavigationPath` (in `NotesView`) so it can push straight into the editor after creating a note, and resets it when its section picker (My Notes / Shared / Tags / Trash) changes.
...
Note *content* is handled separately by `NoteContentService`, which mirrors the Drive app's upload/download encryption protocol exactly (XChaCha20-Poly1305 secretstream for the body, `crypto_box_seal` for the per-file key) so notes are readable by the web app and vice versa. `NoteEditorView` provides live Markdown editing over `MarkdownTextEditor` — a `UITextView` wrapper, used instead of SwiftUI's `TextEditor` because the editor's typing aids need the caret and the keystroke behind an edit, neither of which `TextEditor` exposes. Those aids are checklist continuation (`MarkdownChecklist`: Return carries `- [ ] ` on to the next line, and clears the marker on an item left empty) and the `/` format menu (`MarkdownSlashCommand` + `MarkdownFormat` + `SlashCommandMenu`: a slash typed at the start of a line opens a caret-anchored menu of common Markdown — headings, bold, lists, code — which narrows as you keep typing, so `/h1` is three keystrokes to a heading). On top of them sit debounced autosave via the Drive `PUT /files/{id}/autosave` endpoint, undo/redo, word/character counts and estimated reading time (`NoteTextStats`), and UIKit's find & replace.

Markdown rendering (`NeutrinoNotes/Rendering/`) is layered on top of Apple's [`swift-markdown`](https://github.com/swiftlang/swift-markdown) package, which wraps `cmark-gfm` for CommonMark + GFM tables/strikethrough/task-lists. `MarkdownParser` walks the resulting AST into a pure-Swift, unit-testable model (`MarkdownModel.swift`) and layers a hand-rolled pre/post-processing pass on top for footnotes (`[^1]` refs and `[^1]: ...` definitions), which aren't part of GFM. `MarkdownInlineRenderer` turns inline runs into `AttributedString` (bold/italic/strikethrough/code/links/footnote markers), and `MarkdownView` walks the block model into native SwiftUI (headings, nested lists, read-only task-list checkboxes, `Grid`-based tables, code blocks, block quotes, thematic breaks, `AsyncImage`, tappable links, and a footnotes section). `NoteEditorView` exposes this behind a minimal Preview toggle in its toolbar that swaps the text editor for `MarkdownView`; this is intentionally a placeholder for Epic 7's Edit/Preview/Split View mode switching, not a finished UI.

Organization (Epic 12) reuses Drive's own organizational model rather than inventing one. **Favorites** are Drive's `isStarred` flag on the file or folder itself (`NotesDriveService.setStarred`, `GET /api/v1/drive?view=starred`), so a star set on iOS appears in the web Drive UI and vice versa; **Recents** is the server's `view=recent` listing, so a note edited in the web app shows up on the next refresh. Neither view can be filtered server-side to notes — the API checks its `type` parameter before it looks at `view` and returns early — so both responses are reduced to folders and Markdown files client-side. **Tags** (`TagsService`, `TagsView`, `TaggedNotesView`, `TagPickerSheet`) are full CRUD over `/api/v1/drive/tags` plus per-note assignment; there is no bulk "tags for these files" endpoint, so tags are shown where a single request is natural (the tag browser, the editor's tag chip row, the picker) rather than per row of a listing. The tag list shows each tag's file count as the server reports it, and a tag's notes are fetched a page at a time, since the listing endpoint truncates at 50 by default. The picker's search field filters the loaded tags client-side and doubles as the create field; saving writes one idempotent `POST`/`DELETE /files/{id}/tags/{tag_id}` per changed tag rather than a replace-all `PUT`, so a tag attached on another device while the sheet was open is not silently dropped. **Note that tag names, like note titles, are stored on the server unencrypted** — note bodies remain end-to-end encrypted, but a tag is metadata the server can read. **Pinning** (`PinStore`) is the one piece with no server side at all: Drive has no `is_pinned` column and the web app has no pin concept, so pins are stored in `UserDefaults` on this device only and simply float items to the top of the list they appear in. Favorites is the cross-device mechanism; a pin is a per-device convenience, and the UI labels it as such. One consequence of starring worth knowing: Drive bumps a file's `updated_at` on a metadata write, which the offline sync layer would otherwise read as a remote content edit, so `NotesDriveService` re-bases any pending offline edit through `OfflineStore.rebasePendingEdit` after a successful rename or star — unless the cache already knows about something newer, in which case a genuine conflict is still raised.

Sharing (Epic 22 — `SharingService`, `ShareSheet`, the Notes tab's **Shared** section) reuses Drive's permissions, directory, and encryption-key APIs. Sharing an end-to-end encrypted note is two operations and both have to happen: a permission grant (`POST /drive/{files,folders}/{id}/permissions`) lets the recipient fetch the bytes, and the note's DEK — unsealed on this device with the Keychain key pair and re-sealed to the recipient's public key from `GET /auth/users/{id}/public-key` — is handed to `POST /drive/files/{id}/key/share` so they can read them. The server never sees the DEK. The consequence is that **somebody who has not imported their encryption keys cannot be given a readable note**: there is no public key to seal to. Rather than swallow that as the web dialog does, their row in the share sheet says so and a **Send Key** action re-wraps once they have imported one. There is deliberately no public share link: Drive's link endpoints hand out the stored bytes, which for a note is ciphertext a link recipient has no way to decrypt.

Two things about Drive's shape drive the rest of the design. First, **only owners** may list or change permissions (the API answers 403 to anyone else), and every Drive listing this app calls is owner-scoped server-side — so anything reached through My Notes, Recents, Favorites or Tags is owned by this account, anything reached through `GET /drive/shared-with-me` is not (`NoteItem.isShared`), and the share sheet is offered only for the former. Second, **someone else's folder cannot be listed at all** (`find_folder` filters on the caller's user id), so sharing a *folder* also grants the same role on every note inside it and re-wraps each note's key — those notes then appear as individual rows in the recipient's Shared section, which is the only way they would ever be reachable. A note added to the folder afterwards needs the sheet's **Re-share Folder Contents**, because a DEK can only be re-sealed on a device that can unseal it. A shared note opens read-only until `GET /drive/files/{id}/info` reports `yourRole` as owner or editor, and the actions Drive scopes to owners (Favorites, rename, version history) are disabled on shared notes, while tagging — which *is* permission-aware, with per-user tags — stays available to editors. That same `/info` endpoint replaced the parent-folder listing `NoteContentService.fetchServerModifiedAt` used to rely on: for a note shared with the user that folder belongs to somebody else and 404s, which had silently disabled the offline queue's conflict check for exactly the notes two people might edit at once.
