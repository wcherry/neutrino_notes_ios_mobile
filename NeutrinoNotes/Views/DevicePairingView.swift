...
//
// The receiving half of the two-QR handshake. This device shows QR-A, the other
// device scans it and shows QR-B, this device scans that back.
//
// The old flow was one QR carrying the whole keypair behind a short PIN, which
// meant anyone who photographed the screen held the identity after a few seconds
// of offline guessing. Here the first code is only a public key: photographing
// either code yields nothing without the ephemeral secret half, which never
// leaves this device.
//
// The confirmation code at the end is the one step that cannot be skipped. A
// passive photographer is already defeated; an *active* relay — someone showing
// their own QR-A to the sender and re-sealing to us — is not, and the code is
// what exposes them, because it is derived from a transcript a relay cannot make
// agree on both ends. So the final screen asks the user to compare, and says
...
struct DevicePairingView: View {
    ...
    private func handleScan(_ payload: String) {
        ...
        do {
            ...
            // The account check happens here, before anything is shown as
            // acceptable — a keyring for a different account must not reach the
            // confirmation screen at all.
            Task { @MainActor in
    ...
    private static func qrImage(from string: String) -> UIImage? {
        ...
        // Medium correction: the payload is a sealed keyring and can run to a
        // few hundred bytes, so the extra redundancy of higher levels would push
        // the code denser than a phone camera reads comfortably.
