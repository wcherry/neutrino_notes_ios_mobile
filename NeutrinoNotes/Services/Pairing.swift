...
// MARK: - Pairing
//
// Receiving the keyring from a device that already has it, offline, in two QR
// codes. This device is always the **receiver**; the web client is the sender.
//
...
//
// This replaces the old PIN-protected QR, which put the whole keypair behind a
// few digits — anyone who photographed that code held the identity after a few
// seconds on a GPU. Here, photographing either code yields nothing: QR-A is a
// public key, and QR-B can only be opened by the ephemeral secret half, which
...
struct PairingOffer: Codable {
    ...
    static let type = "neutrino-pair-offer"
}
...
struct PairingResponse: Codable {
    ...
    static let type = "neutrino-pair-response"
}
...
enum Pairing {
    ...
    static func createSession() -> PairingSession {
    ...
    static func encode(_ offer: PairingOffer) throws -> String {
        String(data: try JSONEncoder().encode(offer), encoding: .utf8) ?? ""
    }

    // MARK: Step 3 — accepting the reply

    static func parseResponse(_ raw: String) throws -> PairingResponse {
    ...
    ///
    /// Rejects a response whose nonce is not this session's — that is a QR left
    /// on screen from an earlier attempt, and opening it would install a keyring
    ...
    /// ciphertext actually exchanged — so a relay that substituted its own
    /// ephemeral key cannot make both ends agree. Length-prefixed because the
    /// parts are variable-length and must not be able to slide across each other.
    static func confirmationCode(offer: PairingOffer, response: PairingResponse) -> String {
        var input: [UInt8] = []
        ...
        // Fold four bytes into a number, then take the low digits. Modulo bias
        // across 2^32 into 10^6 is negligible and the code is not a secret — it
        // only has to differ when the transcripts differ.
