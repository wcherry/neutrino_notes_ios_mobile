...
///
/// Drive is not consistent about this, because different endpoints serialize different Rust
/// types. File and folder payloads carry `NaiveDateTime` — no zone, microsecond precision
/// (`2026-07-30T14:25:36.123456`). Version payloads carry `DateTime<Utc>` — RFC 3339 with a
/// zone and 0, 3, 6, or 9 fractional digits depending on the value
/// (`2026-07-30T14:25:36.123456789Z`).
///
/// Rather than listing a `DateFormatter` per shape and hoping the first match wins, the input
/// is split into date-time / fraction / zone, the fraction normalized to milliseconds, and the
/// result parsed with exactly one formatter. Zone-less timestamps are read as UTC, which is
...
enum DriveDate {
    ...
    static func makeDecoder(convertFromSnakeCase: Bool = false,
